...
{ 																		//Allows for access to Soomla API										
	...
	{	
		...
		void CheckIAP_PurchaseStatus(){
			Debug.Log (StoreInventory.GetItemBalance("bspack_item_id"));							// Print the current status of the IAP
			if (StoreInventory.GetItemBalance("bspack_item_id") >= 1) 
			{
		...
		public void buy_BS(){
			try {
				...
				StoreInventory.BuyItem ("bspack_item_id");										// if the purchases can be completed sucessfully
			} 
			catch (Exception e) 
